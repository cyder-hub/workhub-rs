...
The setup wizard reads existing global `.env` values first. If a service already has config, it prints a redacted summary and asks whether to modify that service. Jira and Confluence choose Cloud username/API-token auth for `*.atlassian.net` URLs. For Server/Data Center URLs, the wizard asks whether to configure personal-token auth or username/password auth. In an interactive terminal, confirmations, text inputs, secret inputs, and auth-method choices use dialoguer prompts; invalid input stays on the prompt until corrected. Configured auth methods are marked with `[configured]`, and incomplete methods are marked with `[partial]`. `workhub cli config setup atlassian` asks for Cloud username/API-token, Server/Data Center personal-token, or Server/Data Center username/password shared credentials. GitLab prompts for one primary token. Existing secret values are kept when the prompt is left blank. Optional values such as `*_SSL_VERIFY`, `*_TIMEOUT`, and project/space filters are prompted only when they already exist; add new advanced values with `config set`.
