...
Use these commands for GitLab user/project lookup and merge request review workflows.

The `<project>` argument can be a numeric project ID or a full path such as `group/project` or `group/subgroup/project`.

## Input Payloads

Merge request descriptions and note/discussion bodies can come from inline text, a file, or stdin. Merge commit messages and squash commit messages are inline text flags.

## Users And Projects

| Command | Use | Example | Returns |
| --- | --- | --- | --- |
| `cli gitlab user current` | Show the current GitLab user. | `workhub cli gitlab user current` | Text user fields by default; user object with `--json`. |
...
| `cli gitlab mr close <project> <iid> --confirm-iid <iid>` | Close a merge request through an explicit cleanup command. | `workhub cli gitlab mr close group/project 7 --confirm-iid 7` | Mutation envelope with cleanup hint; use `--json` for exact fields. |
| `cli gitlab mr delete <project> <iid> --confirm-iid <iid>` | Delete a merge request when the GitLab instance and token allow it. | `workhub cli gitlab mr delete group/project 7 --confirm-iid 7` | Mutation envelope with cleanup hint; 405 maps to `unsupported_or_auth_required` and `gitlab mr close` remains the fallback cleanup path. |
| `cli gitlab mr note add <project> <iid> (--body <text> \| --body-file <path> \| --body-stdin)` | Add a merge request note and discover its discussion id when possible. | `workhub cli gitlab mr note add group/project 7 --body-file ./mr-note.md` | New note summary by default; note object with `discussion_id` using `--json` when discovery succeeds. |
| `cli gitlab mr note update <project> <iid> <note-id> (--body <text> \| --body-file <path> \| --body-stdin)` | Update a merge request note. | `workhub cli gitlab mr note update group/project 7 239 --body 'Updated note'` | Updated note summary by default; note object with `--json`. |
| `cli gitlab mr note delete <project> <iid> <note-id>` | Delete a merge request note. | `workhub cli gitlab mr note delete group/project 7 239` | Cleanup summary by default; operation result with `--json`. |
| `cli gitlab mr discussion list <project> <iid> [--page <n>] [--per-page <n>]` | List merge request discussions and their note ids. | `workhub cli gitlab mr discussion list group/project 7 --per-page 50` | Discussion list by default; discussion objects with note ids using `--json`. |
...
| `cli gitlab branch create <project> <branch> --ref <ref>` | Create an isolated branch for a merge request workflow. | `workhub cli gitlab branch create group/project feature/workhub-cli --ref main` | Branch object with `--json`; project and branch path segments are encoded by the client. |
| `cli gitlab branch delete <project> <branch> --confirm-branch <branch>` | Delete an isolated branch after cleanup. | `workhub cli gitlab branch delete group/project feature/workhub-cli --confirm-branch feature/workhub-cli` | Mutation envelope with cleanup hint; `--confirm-branch` must exactly match the branch argument. |

## Safety Notes

- Run `cli gitlab mr get <project> <iid>` before merging to verify state, target branch, and latest SHA.
- Use `cli gitlab mr discussion list <project> <iid>` to get real discussion ids. Do not use a note id as a discussion id.
- Use `cli gitlab mr close` as the default cleanup path. Use `cli gitlab mr delete` only for disposable test MRs and only when `--confirm-iid` exactly matches the IID.
- Use branch create/delete only for isolated test branches. `cli gitlab branch delete` requires `--confirm-branch` exactly equal to the branch argument.
